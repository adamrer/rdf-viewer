...
function isContainer(context, itemIri) {
  ...
  return types.some(
    (sourcedObject) => sourcedObject.term.value === ldp + "Container",
  );
}
function createListItem(context, itemIri) {
  ...
  if (label) {
    ...
    itemElement.appendChild(createLabelHtmlFromIri(itemIri));
  }
  return itemElement;
}
function createLabelHtmlFromIri(iri) {
  ...
  labelSpan.textContent = getResourceName(iri);
...
function getResourceName(resourceIri) {
  ...
  if (split.length === 0) return resourceIri;
