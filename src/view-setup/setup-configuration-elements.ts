...
function setupPluginList() {
  ...
  new Sortable(pluginListEl, {
    ...
    onEnd: (evt) => {
      const order: number[] = Array.from(evt.to.children)
...
function setupDataSourceForm() {
  ...
  const select = addSourceFormEl.querySelector(
    "#data-source-select",
  ) as HTMLSelectElement;
  const urlTextInput = addSourceFormEl.querySelector(
  ...
  select.addEventListener("change", () => {
    ...
    switch (dataSourceType) {
      case DataSourceType.Sparql:
        if (urlTextInput.disabled) switchInput(urlTextInput, fileInput);
        break;
      case DataSourceType.RemoteFile:
        if (urlTextInput.disabled) switchInput(urlTextInput, fileInput);
        break;
      case DataSourceType.LocalFile:
        if (fileInput.disabled) switchInput(urlTextInput, fileInput);
        break;
...
function addDataSourceFromFormData(formData: FormData) {
  const app = RdfViewerState.getInstance();
  const dsType: DataSourceType = formData.get(
    "data-source-select",
  ) as DataSourceType;
  switch (dsType) {
    case DataSourceType.Sparql: {
...
function setupPluginForm() {
  ...
  const select = addPluginFormEl.querySelector(
    "#plugin-add-type-select",
  ) as HTMLSelectElement;
  const urlInput = addPluginFormEl.querySelector(
    "#plugin-add-url-input",
  ) as HTMLInputElement;
  ...
  select.addEventListener("change", () => {
    const pluginType: PluginModuleImportType =
      select.value as PluginModuleImportType;
    switch (pluginType) {
      case PluginModuleImportType.Url:
        if (urlInput.disabled) switchInput(urlInput, fileInput);
        break;
      case PluginModuleImportType.File:
        if (fileInput.disabled) switchInput(urlInput, fileInput);
        break;
      default:
        break;
    }
...
async function addPluginsFromFormData(formData: FormData) {
  const app = RdfViewerState.getInstance();
  const pluginType: PluginModuleImportType = formData.get(
    "plugin-add-type",
  ) as PluginModuleImportType;
  switch (pluginType) {
    case PluginModuleImportType.Url: {
      const url = formData.get("url-input") as IRI | null;
      if (!url) throw new Error("Missing plugin url");
...
export { setupConfigurationElements };
