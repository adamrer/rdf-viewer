...
function createLdpExplorerPlugin() {
  return {
    ...
    async isCompatible(context, subject) {
      ...
      const containerTypes = context.data.vocabulary.getSemanticallySimilar(
        ldp.Container,
      );

      const isCompatible = typeValues.some(
        (t) => containerTypes.includes(t) || t === ldp.BasicContainer,
      );
      ...
      return {
        mount(element) {
          ...
          (async () => {
            ...
            const members = context.data.fetched
              .subject(subject)
            ...
            for (const memberIri of sortedMembers) {
              ...
              if (isSubContainer) {
                ...
                toggleBtn.onclick = async () => {
                  ...
                  if (isOpening) {
                    ...
                    const handler =
                      await context.interoperability.renderSubject(
                        memberIri,
                        subContentDiv,
                      );
                    if (handler) {
                      ...
                      subContentDiv.textContent =
                        "No plugin found for this folder.";
                    }
                  } else {
              ...
              nameText.textContent =
                labelSplit[labelSplit.length - 2] ??
...
async function isContainer(iri, context) {
  ...
  return types.some(
    (t) =>
      containerTypes.includes(t.value.value) ||
      t.value.value === ldp.BasicContainer,
  );
}
