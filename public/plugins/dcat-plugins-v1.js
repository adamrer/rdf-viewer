...
function createDatasetPlugin() {
  return {
      ...
      return {
        mount(element) {
          ...
          (async () => {
            ...
            await context.data.fetch.quads(
              [subject],
              undefined,
              context.configuration.languages,
            );
            await context.data.fetch.quads(
              [dcat.distribution, ...chosenPredicates],
              labelPredicates,
              context.configuration.languages,
            );
            ...
            chosenPredicates.forEach((predicate) => {
              const objects = context.data.fetched
                .subject(subject)
                .predicate(predicate);
            ...
            // Render Title (h1)
            const datasetLabel = getLabel(
              subject,
              [dcterms.title, skos.prefLabel, rdfs.label],
              context,
            );
            ...
            descriptionElement.textContent =
              context.data.fetched
                .subject(subject)
                .predicate(dcterms.description)[0]?.value?.value ??
              "No description";
            ...
            const distributions = context.data.fetched
              .subject(subject)
              .predicate(dcat.distribution);
            const distributionIris = distributions
              .filter((q) => q.value.termType === "NamedNode")
              .map((q) => q.value.value);
            ...
            distributionIris.forEach(async (iri) => {
              ...
              const handler = await context.interoperability.renderSubject(
                iri,
                distributionElement,
              );
              if (handler == null) {
    ...
    async isCompatible(context, subject) {
      ...
      const datasetTypeIris = context.data.vocabulary.getSemanticallySimilar(
        dcat.Dataset,
      );
...
function createDistributionPlugin() {
  return {
      ...
      return {
        mount(element) {
          ...
          (async () => {
            ...
            const formats = context.data.fetched
              .subject(subject)
            ...
            const distributionLabel = getLabel(
              subject,
              [dcterms.title, skos.prefLabel, rdfs.label],
              context,
            );
            ...
            if (subject !== distributionLabel) {
              titleElement.appendChild(
                createLabelElement(subject, distributionLabel),
              );
              fieldset.appendChild(titleElement);
            }

            // Render Format/MediaType fields
            const predicatesDl = createPredicatesDescriptionList(
              subject,
              [dcterms.format, dcat.mediaType],
              context,
            );
            // Render Download/Access links
            const linksDl = createLinksDescriptionList(
              subject,
    ...
    async isCompatible(context, subject) {
      const subjectTypes = await context.data.fetch.types(subject);
      const subjectTypeValues = subjectTypes.map((t) => t.value.value);
      const distributionTypeIris =
        context.data.vocabulary.getSemanticallySimilar(dcat.Distribution);
...
function getLabel(subject, labelPredicates, context) {
  ...
  if (labelLiterals.length !== 0)
    return (
      getByLanguagePriority(labelLiterals, context.configuration.languages) ??
      subjectLabel
    );
  return subjectLabel;
}
...
function createLinksDescriptionList(subject, linkPredicates, context) {
  ...
  linkPredicates.forEach((predicate) => {
    ...
    if (objects.length !== 0) {
      const downloadElement = createLabelElement(
        predicate,
        getLabel(predicate, [rdfs.label], context),
      );
...
function createPredicatesDescriptionList(subject, predicates, context) {
  ...
  const descriptionTerms = predicates.map((predicate) => {
    const dt = document.createElement("dt");
    dt.appendChild(
      createLabelElement(
        predicate,
        getLabel(predicate, labelPredicates, context),
      ),
    );
    dt.style.fontWeight = "bold";
    ...
    const descriptionDefinitions = objects.map((object) => {
      ...
      if (predicate === dcat.contactPoint) {
        const handlerPromise = context.interoperability.renderSubject(
          object.value.value,
          dd,
          minContactPointPriority,
        );
        handlerPromise.then((handler) => {
          if (handler === null) {
            dd.appendChild(
              createLabelElement(
                object.value.value,
                getLabel(object.value.value, labelPredicates, context),
              ),
            );
          }
        });
