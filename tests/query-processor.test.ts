...
test("processes query with more subjects specified", () => {
  ...
  expect(
    resultQuads.every(
      (quad) => quad.subject.value !== "http://www.w3.org/ns/dcat#theme",
    ),
  ).toBe(true);
});
...
test("processes query with filter to get non-empty result", () => {
  ...
  const processor = queryProcessor();
  const resultQuads = processor.filter(quads, query);
  const languageIsRight = resultQuads.every(
    (quad) =>
